...
// You might still have a TimeSlotBooking.css, or you can integrate the styles directly.
// If you want to use the CSS file from my previous example, create it:
// import './TimeSlotBooking.css'; // Uncomment if you create the CSS file
// Otherwise, the inline styles at the bottom of this file will be used.
...
const TimeSlotBooking = () => {
    ...
    const fetchAvailableSlots = useCallback(async () => {
        ...
        const formattedDate = formatDate(selectedDate); // Ensure date is formatted for the backend query
        // --- IMPORTANT CHANGE 1: Updated the API endpoint for fetching slots ---
        const backendUrl = `https://mirute-backend-api.onrender.com/api/booking-slots?date=${formattedDate}`;
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Failed to fetch slots.'); // Use .message as per your backend
            }
    ...
    const handleBookingSubmit = async (e) => {
        ...
        try {
            // --- IMPORTANT CHANGE 2: Updated the API endpoint for booking a slot ---
            const response = await fetch(`https://mirute-backend-api.onrender.com/api/booking-slots/book`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    // Your backend only uses slotId for booking.
                    // customerName, customerEmail, customerPhone, serviceType, notes
                    // would need to be added to the backend BookingSlot schema and logic
                    // if you want to store them with the booked slot.
                    // For now, these are only sent if the backend is updated to receive them.
                }),
            });

            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.message || 'Booking failed. Please try again.'); // Use .message as per your backend
            }
            ...
            setBookingMessage(`Booking failed: ${err.message || 'An unexpected error occurred during booking.'}`);
        } finally {
            setLoading(false);
        }
    };
...
// Inline styles (you can move these to a separate CSS file if preferred)
const styles = {
    container: {
